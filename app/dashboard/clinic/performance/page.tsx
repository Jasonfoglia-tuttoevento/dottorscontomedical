import ClinicVisibilityAnalytics from "@/components/clinics/analytics/ClinicVisibilityAnalytics";
...
import { getClinicAnalyticsData } from "@/lib/data/clinic-analytics";
...
export default async function ClinicPerformancePage() {
  ...
  const [analytics, dashboard] = await Promise.all([
    getClinicAnalyticsData(clinic.id, 30),
    getClinicDashboardData(clinic),
  ]);

  return (
    <div className="mx-auto max-w-7xl space-y-10">
      <ClinicVisibilityAnalytics data={analytics} />

      <section className="space-y-6 border-t border-slate-200 pt-10">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.16em] text-[#0D47A1]">Analisi commerciale</p>
          <h2 className="mt-1 text-2xl font-black text-slate-950 sm:text-3xl">Richieste e conversioni</h2>
          <p className="mt-2 text-sm text-slate-600">Dalla visibilità alle richieste effettivamente gestite dalla clinica.</p>
        </div>
